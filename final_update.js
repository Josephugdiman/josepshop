...
// Fix encoding issues broadly
content = content.replace(/Ã¢Å“â€¦/g, '✅');

// The new handeUpdateVendor logic
const newFunc = `
...
const startMarker = '  const handleUpdateVendor = async () => {';
const renewMarker = '  const handleRenewVendor = (vendor) => {';

const startIndex = content.indexOf(startMarker);
const renewIndex = content.indexOf(renewMarker);

if (startIndex !== -1 && renewIndex !== -1 && renewIndex > startIndex) {
  const finalContent = content.substring(0, startIndex) + newFunc + "\n\n" + content.substring(renewIndex);
  fs.writeFileSync(filePath, finalContent);
  console.log("SUCCESS_UPDATE_HANDLE");
} else {
  console.log("FAILED_TO_LOCATE_BLOCKS", { startIndex, renewIndex });
}
